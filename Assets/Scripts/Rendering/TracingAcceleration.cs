...
namespace Rendering
{
    ...
    public class AABB
    {
        ...
        {
            return 2.0f * (pExtent.x * pExtent.y + pExtent.x * pExtent.z + pExtent.y * pExtent.z);
        }
    ...
    public abstract class BVH
    {
        ...
        public class BVHNode
        {
            ...
            public static BVHNode InitLeaf(int start, int count, AABB bounding) => new BVHNode
            {
                Bounds = bounding,
                LeftChild = null,
                RightChild = null,
            ...
            public static BVHNode InitInterior(int splitAxis, BVHNode nodeLeft, BVHNode nodeRight) => new BVHNode
            {
        ...
        /// <returns></returns>
        protected abstract BVHNode Build(List<FaceInfo> faceInfo, int faceInfoStart, int faceInfoEnd);
        ...
        public void FlattenBLAS(ref List<int> indices, ref List<BLASNode> bnodes, ref List<TLASRawNode> tnodesRaw, List<int> subindices, int verticesOffset, int materialIdx, int transformIdx)
        {
        ...
        protected List<FaceInfo> CreateFaceInfo(List<Vector3> vertices, List<int> indices)
        {
            ...
            for (int i = 0; i < indices.Count / 3; i++)
            {
                ...
                {
                    Bounds = new AABB(vertices[indices[i * 3]], vertices[indices[i * 3 + 1]], vertices[indices[i * 3 + 2]]),
                    FaceIdx = i
                });
        ...
        protected List<FaceInfo> CreateFaceInfo(List<TLASRawNode> rawNodes, List<Matrix4x4> transforms)
        {
            ...
            for (int i = 0; i < rawNodes.Count; i++)
            {
                ...
                info.Add(new FaceInfo
                {
                    Bounds = new AABB(transforms[node.TransformIdx * 2].MultiplyPoint3x4(node.BoundMin), transforms[node.TransformIdx * 2].MultiplyPoint3x4(node.BoundMax)),
                    FaceIdx = i
                });
                info[i].Center = info[i].Bounds.Center();
            }
        ...
        public static BVH Construct(List<Vector3> vertices, List<int> indices, BVHType type) => type switch
        {
            BVHType.Naive => new BVHNaive(vertices, indices),
            BVHType.SAH => new BVHSAH(vertices, indices),
            _ => new BVHNaive(vertices, indices)
        };

        public static BVH Construct(List<TLASRawNode> rawNodes, List<Matrix4x4> transforms, BVHType type) => type switch
        {
            BVHType.Naive => new BVHNaive(rawNodes, transforms),
            BVHType.SAH => new BVHSAH(rawNodes, transforms),
            _ => new BVHNaive(rawNodes, transforms)
        };
    }
    ...
    public class BVHSAH : BVH
    {
        ...
        protected sealed override BVHNode Build(List<FaceInfo> faceInfo, int faceInfoStart, int faceInfoEnd)
        {
            ...
            else
            {
                ...
                else
                {
                    if (faceInfoCount <= 2)
                    {
                        ...
                        faceInfo.Sort(faceInfoStart, faceInfoCount, Comparer<FaceInfo>.Create((x, y) => x.Center[dim].CompareTo(y.Center[dim])));
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < nBuckets - 1; i++)
                        {
                            ...
                            float cost = countBelow[i] * boundsBelow[i].SurfaceArea() + countAbove[i] * boundsAbove[i].SurfaceArea();
                            if (cost < minCost)
                            {
                                minCost = cost;
                    ...
                    return BVHNode.InitInterior(dim, leftChild, rightChild);
                }
            }
        }
    }
