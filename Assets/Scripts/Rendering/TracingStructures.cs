...
namespace Rendering
{
    ...
    public class ObjectManager : IDisposable
    {
        ...
        private static List<Matrix4x4> transforms = new List<Matrix4x4>(); // size of objects * 2, local to world & world to local transform
        ...
        public static void ReloadTLAS()
        {
            ...
            tnodes.Clear();
            BVH.Construct(tnodesRaw, transforms, BVHConstructorType).FlattenTLAS(ref tnodesRaw, ref tnodes);
            TLASBuffer.UpdateBuffer(tnodes, TLASNode.TypeSize);
            TLASRawBuffer.UpdateBuffer(tnodesRaw, TLASRawNode.TypeSize);
        }
        ...
        private static bool BuildBVH()
        {
            ...
            for (int idx = 0; idx < objects.Count; idx++)
            {
                ...
                var matStart = materials.Count;
                var matCount = meshMats.Length;
                foreach (var mat in meshMats)
                {
                    ...
                    materials.Add(new MaterialData
                    {
                        Color = mat.color.ToVector4(),
                        Emission = mat.IsKeywordEnabled("_EMISSION") ? mat.GetColor("_EmissionColor").ToVector3() : Vector3.zero,
                ...
                for (int i = 0; i < mesh.subMeshCount; i++)
                {
                    ...
                    var blasTree = BVH.Construct(meshVertices, submeshIndices, BVHConstructorType);
            ...
            IndexBuffer.UpdateBuffer(indices, sizeof(int));
            VertexBuffer.UpdateBuffer(vertices, sizeof(float) * 3);
            UVBuffer.UpdateBuffer(uvs, sizeof(float) * 2);
            NormalBuffer.UpdateBuffer(normals, sizeof(float) * 3);
            TangentBuffer.UpdateBuffer(tangents, sizeof(float) * 4);
            MaterialBuffer.UpdateBuffer(materials, MaterialData.TypeSize);
            BLASBuffer.UpdateBuffer(bnodes, BLASNode.TypeSize);
            ...
            if (NormalTextures != null) UnityEngine.Object.Destroy(NormalTextures);
            if (RoughnessTextures != null) UnityEngine.Object.Destroy(RoughnessTextures);
            AlbedoTextures = albedoTex.ToTextureArray();
            EmissionTextures = emitTex.ToTextureArray();
            MetallicTextures = metalTex.ToTextureArray();
            NormalTextures = normTex.ToTextureArray();
            RoughnessTextures = roughTex.ToTextureArray();
        ...
        private static bool LoadTransforms()
        {
            ...
            TransformBuffer.UpdateBuffer(transforms, sizeof(float) * 4 * 4);
